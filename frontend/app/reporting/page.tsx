...
/*──────────────────────────────────────────────────────────────────────────
  Dynamic imports for all charts that can appear in a report
  (keep or comment-out rows as needed)
──────────────────────────────────────────────────────────────────────────*/
type ChartProps = { width?: number; height?: number }
...
const ChartComponents = {
  ...
  "applicant-analysis"             : dynamic<ChartProps>(() => import("@/components/applicant_type_pie").then(m => m.default || m),     { ssr:false }),
  "market-size"                    : dynamic<ChartProps>(() => import("@/components/IPStat").then(m => m.default || m),                 { ssr:false }),
  "top-ipc"                        : dynamic(() => import("@/components/top_ipc_codes").then(m => m.default || m),                      { ssr:false }),
  "geographic-distribution"        : dynamic(() => import("@/components/geographical_distribution").then(m => m.default || m),            { ssr:false }),
  "top-10-applicants"              : dynamic(() => import("@/components/top_10_app").then(m => m.default || m),                         { ssr:false }),
  "top-10-keywords"                : dynamic(() => import("@/components/top_10_keywords").then(m => m.default || m),                    { ssr:false }),
  "patent-field-trends"            : dynamic(() => import("@/components/patent_by_field").then(m => m.default || m),                    { ssr:false }),
  "evolving-word-cloud"            : dynamic(() => import("@/components/evolving_word_cloud").then(m => m.default || m),                { ssr:false }),
  "cooccurrence-trends"            : dynamic(() => import("@/components/co-occurunece_trend").then(m => m.default || m),                { ssr:false }),
  "applicant-collaboration-network": dynamic(() => import("@/components/collaboration_network").then(m => m.default || m),              { ssr:false }),
  "originality-rate"               : dynamic(() => import("@/components/originality_rate").then(m => m.default || m),                   { ssr:false }),
  "family-member-count"            : dynamic(() => import("@/components/family-member-count").then(m => m.default || m),                { ssr:false }),
  "family-size-distribution"       : dynamic(() => import("@/components/family-size-distribution").then(m => m.default || m),           { ssr:false }),
  "international-protection-matrix": dynamic(() => import("@/components/international-protection-matrix").then(m => m.default || m),    { ssr:false }),
  "international-patent-flow"      : dynamic(() => import("@/components/international-patent-flow").then(m => m.default || m),          { ssr:false }),
  "ipstat"                         : dynamic(() => import("@/components/IPStat").then(m => m.default || m),               { ssr:false }),
/* reporting/page.tsx  – inside ChartComponents */

"innovation" : dynamic<ChartProps>(
  () => import("@/components/innovation-cycle").then(m => m.InnovationCycle),
  { ssr: false }
),

"investment"                     : dynamic(() => import("@/components/investment_dynamic").then(m => m.default || m),   { ssr:false }),
"originality"                    : dynamic(() => import("@/components/originality_rate").then(m => m.default || m),     { ssr:false }),
"analysis-summary"               : dynamic(() => import("@/components/analysis-summary").then(m => m.default || m),     { ssr:false }),
} as const
...
const availableCharts: { id: ChartId; title: string; type: string }[] = [
  ...
  { id: "international-patent-flow",       title: "International Patent Flow",       type: "summary" },
  // >>> NEW – summary cards
{ id: "ipstat",               title: "IP Stats",            type: "summary" },
{ id: "innovation",           title: "Innovation Cycle",    type: "summary" },
{ id: "investment",           title: "Investment Dynamic",  type: "summary" },
{ id: "originality",          title: "Originality Rate",    type: "summary" },
{ id: "analysis-summary",     title: "Analysis Summary",    type: "summary" },
...
export default function Reporting() {
  ...
  const updateComment = (id: ChartId, txt: string) =>
    setChartComments(prev => ({ ...prev, [id]: txt }))
  ...
  const generatePptx = async () => {
    ...
    if (!divs.length) return alert("No charts to export.")
    const images = await Promise.all(
      divs.map(async d => ({ id: d.id, data: (await html2canvas(d)).toDataURL("image/png") }))
    )
    try {
  ...
  /*───────────────────────────  NEW: PDF export via jspdf  ───────────────────────*/
  const generatePdf = async () => {
    const divs = Array.from(document.querySelectorAll<HTMLDivElement>('[id^="chart-"]'))
    if (!divs.length) return alert("No charts to export.")

    // grab screenshots first
    const images = await Promise.all(
      divs.map(async d => ({
    ...
    // lazy-import jspdf to avoid SSR issues
    const { jsPDF } = await import("jspdf")
    const doc = new jsPDF({ orientation: "landscape", unit: "pt", format: "a4" })
    const pageW = doc.internal.pageSize.getWidth()
    const maxImgW = pageW - 80 // 40pt margin on each side

    images.forEach((img, idx) => {
      if (idx > 0) doc.addPage()

      // scale image to fit page width, keep aspect ratio (assume 640×400)
      const imgW = maxImgW
      const imgH = (400 / 640) * imgW
      doc.addImage(img.data, "PNG", 40, 40, imgW, imgH)

      const comment = (chartComments as Record<string, string>)[img.id] || ""
      if (comment) {
        const lines = doc.splitTextToSize(comment, maxImgW)
        doc.setFontSize(12)
        doc.text(lines, 40, 40 + imgH + 20)
      }
    })

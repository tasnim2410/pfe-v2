...
export function SearchResults({ hasSearched, results, papers = [], loading }: SearchResultsProps) {
  ...
  const fmtFields = (x: any) => Array.isArray(x) ? x.join(", ") : (x || "");
  const getPaperTitle = (p: any) => p.title || p.official_title || "";
  const getJournal = (p: any) => p.journal_name || p.publication_venue_name || "";
  const getYear = (p: any) => p.year ?? "";
  const getCitations = (p: any) => p.citation_count ?? "";
  const getRefs = (p: any) => p.reference_count ?? "";
  const getSubcat = (p: any) => p.subcategory ?? "";
  ...
  return (
    <Tabs
      ...
      className="w-full"
      value={activeTab}
      onValueChange={val => setActiveTab(val as 'patents' | 'papers' | 'summary')}
    >
      <TabsList className="grid w-full grid-cols-3">
        ...
        <TabsTrigger
          value="papers"
          className={`${styles.tabTrigger} ${activeTab !== 'papers' ? styles.tabTriggerInactive : ''}`}
        >
          Papers
        </TabsTrigger>
        <TabsTrigger
      ...
      <TabsContent value="papers" className="space-y-4">
        <Card>
          <CardHeader>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
              <CardTitle style={{ margin: 0 }}>Scientific Papers</CardTitle>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <label htmlFor="paperRowCountSelect" style={{ fontWeight: 500, fontSize: 14, marginRight: 4 }}>Show:</label>
                <select
                  id="paperRowCountSelect"
                  value={paperRowCount}
                  onChange={e => setPaperRowCount(e.target.value === 'all' ? 'all' : Number(e.target.value))}
                  style={{ padding: '4px 8px', borderRadius: 4, border: '1px solid #ccc', fontSize: 14 }}
                >
                  <option value={10}>10</option>
                  <option value={20}>20</option>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Fields of Study</TableHead>
                ...
                {papers && papers.length > 0 ? (
                  (paperRowCount === 'all' ? papers : papers.slice(0, paperRowCount)).map((p: any, idx: number) => (
                    <TableRow key={p.paper_id || p.id || idx}>
                      ...
                      <TableCell>{getJournal(p)}</TableCell>
                      <TableCell>{fmtFields(p.fields_of_study)}</TableCell>
                      <TableCell>{getCitations(p)}</TableCell>
                      <TableCell>{getRefs(p)}</TableCell>
                      <TableCell>{getSubcat(p)}</TableCell>
                    </TableRow>
                  ))
                ) : (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center text-gray-500">No papers found</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>

...
const AdvancedSearchBar: React.FC = () => {
  ...
  // --- Build a simple boolean query string for /api/scientific_search_merge ---
  //   We ignore fields/rule_op, and preserve AND/OR with parentheses.
  const buildQueryString = (node: QueryNode): string => {
    if (node.type === 'keyword') {
      const w = (node.word || '').trim();
      if (!w) return '';
      // if it contains spaces or special chars, quote it
      return /\s/.test(w) ? `"${w}"` : w;
    } else {
      const g = node as Group;
      const parts = g.children.map(buildQueryString).filter(Boolean);
  ...
  // Optional: Function to format state for patent endpoint
  ...
  // Formats the payload exactly as the Flask endpoint expects (patents)

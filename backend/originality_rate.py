...
def fetch_and_store_originality_data(limit=50, batch_size=50, max_retries=3, retry_base_delay=1):
    """
    Fetches patent data from raw_patents, retrieves citation numbers and IPCs, 
    calculates ratios, and stores in originality_data table. 
    Commits in batches for performance and resilience.
    Skips patents already present in originality_data.
    Shows a progress bar.
    Retries API failures with exponential backoff.
    """
    from db import db, RawPatent  # update this import for your project
    import pandas as pd

    # Gather already-processed patent numbers to skip
    print("Fetching already processed publication numbers...")
    already = {e.first_publication_number for e in OriginalityData.query.with_entities(OriginalityData.first_publication_number).all()}
    print(f"{len(already)} patents already processed will be skipped.")

    patents = RawPatent.query.with_entities(
        RawPatent.first_publication_number, RawPatent.ipc
    ).limit(limit).all()
    ...
    for idx, (pub_num, ipc) in enumerate(tqdm(patents, desc="Processing patents")):
        if pub_num in already:
            skipped.append(pub_num)
            continue
        if not is_valid_pub_number(pub_num):
            print(f"Skipping invalid pub_num: {pub_num}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"Error fetching API for {pub_num}: {type(e).__name__}: {e}. Retrying in {delay:.2f}s...")
                    sleep(delay)
                else:
                    print(f"Permanent API failure for {pub_num}: {type(e).__name__}: {e}")
                    failed.append(pub_num)
                    citation_numbers, citations_ipc = [], []

        if not citation_numbers or not citations_ipc:
            continue
        ...
        if len(batch) >= batch_size:
            ...
            try:
                db.session.commit()
                print(f"Committed batch at {idx + 1} patents processed.")
            except SQLAlchemyError as e:
                print(f"Batch commit failed at {idx + 1}: {e}")
                db.session.rollback()
            batch = []
    ...
    if batch:
        ...
        try:
            db.session.commit()
            print(f"Final batch committed ({len(batch)} entries).")
        except SQLAlchemyError as e:
    ...
    print(f"OriginalityData: {added} added, {len(skipped)} skipped, {len(failed)} failed")
    if skipped:
        print(f"Skipped publication numbers: {skipped[:10]}{'...' if len(skipped) > 10 else ''}")
    if failed:
        print(f"Failed publication numbers: {failed[:10]}{'...' if len(failed) > 10 else ''}")

...
# Modes:
#   - test   : train ≤ split_year, forecast > split_year, report MAE/RMSE
#   - future : train all, forecast H with future pub path (explicit or scenario)
#
...
split_year  <- as.integer(get_cfg("--split_year", "SPLIT_YEAR", "2015"))
...
df <- data.frame(year = years_all) %>%
  left_join(pat_df, by = "year") %>%
  left_join(pub_df, by = "year") %>%
  mutate(
...
# lag + quadratic exog
df <- df %>%
  mutate(pub_lag1 = dplyr::lag(pub_count, 1)) %>%
  filter(!is.na(pub_lag1)) %>%
  mutate(pub_lag1_sq = pub_lag1^2)
...
if (mode == "test") {
  train_idx <- which(yrs <= split_year)
  test_idx  <- which(yrs >  split_year)
  if (length(test_idx) == 0) {
    cat(toJSON(list(ok = FALSE, stage = "split", error = "No test observations after split_year."), auto_unbox = TRUE))
    quit(save = "no", status = 0)
  }

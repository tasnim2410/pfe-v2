...
# ============================
# Auto-discover inputs inside outputs/
#   expects pairs like:
#     outputs/patents-1.csv, outputs/publications-1.csv, outputs/techs_order-1.txt
# ============================
def discover():
    pat_glob = glob.glob(os.path.join(BASE_DIR, "patents-*.csv"))
    pub_glob = glob.glob(os.path.join(BASE_DIR, "publications-*.csv"))
    ord_glob = glob.glob(os.path.join(BASE_DIR, "techs_order-*.txt"))

    pat_files = {re.findall(r"patents-(.+)\.csv$", os.path.basename(p))[0]: p for p in pat_glob}
    pub_files = {re.findall(r"publications-(.+)\.csv$", os.path.basename(p))[0]: p for p in pub_glob}
    order_files = {re.findall(r"techs_order-(.+)\.txt$", os.path.basename(p))[0]: p for p in ord_glob}
    ids = sorted(set(pat_files) | set(pub_files) | set(order_files))
    if not ids:
        raise RuntimeError(f"No inputs found in {BASE_DIR}/")
    return [(i, pat_files.get(i), pub_files.get(i), order_files.get(i)) for i in ids]
...
# ============================
rows = discover()
pat_frames, pub_frames = [], []

for _id, pat_csv, pub_csv, ord_txt in rows:
    pat_map, pub_map = parse_order_file(ord_txt)
    if pat_csv:
        pat_frames.append(load_kind(pat_csv, pat_map, "pat"))
    if pub_csv:
        pub_frames.append(load_kind(pub_csv, pub_map, "pub"))

pat_merged = outer_merge_by_year(pat_frames)
pub_merged = outer_merge_by_year(pub_frames)
